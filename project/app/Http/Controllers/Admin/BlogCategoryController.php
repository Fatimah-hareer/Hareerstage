...
class BlogCategoryController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
             ->editColumn('slug', function(BlogCategory $data) {
                 $name = mb_strlen(strip_tags($data->slug),'utf-8') > 50 ? mb_substr(strip_tags($data->slug),0,50,'utf-8').'...' : strip_tags($data->slug);
                 return  $name;
             })
             ->editColumn('name', function(BlogCategory $data) {
                 $name = mb_strlen(strip_tags($data->name),'utf-8') > 50 ? mb_substr(strip_tags($data->slug),0,50,'utf-8').'...' : strip_tags($data->name);
                 return  $name;
             })
    ...
    public function store(Request $request)
    {
        ...
        $input['slug'] = [
            'ar'=>$request->input('slug_ar'),
            'en'=>$request->input('slug_en')
        ];
